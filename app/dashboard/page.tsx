      ...
      <div className="mx-auto max-w-6xl">
        <header className="mb-8 flex items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-sm font-bold tracking-tight text-slate-950">Gelir Gider</p>
              <p className="text-xs text-slate-400">Ortak takip alanı</p>
            </div>
          </div>
        ...
        <section className="mb-8 flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
          <div>
            ...
            <h1 className="mt-2 text-3xl font-black tracking-tight text-slate-950 sm:text-4xl">
              Merhaba, {currentMember.name}
            </h1>
            <p className="mt-2 text-sm text-slate-500">
              Gelir ve giderlerini tek yerde, kolayca takip et.
            </p>
          </div>
          <p className="rounded-full bg-white px-4 py-2 text-xs font-semibold text-slate-500 shadow-sm">
            {transactions.length} kayıt
          </p>
        </section>

        <div className="flex flex-col">
          <div className="order-1 mb-8 grid gap-6 lg:order-2 lg:grid-cols-[minmax(0,0.78fr)_minmax(0,1.22fr)] lg:mb-0">
            <section className="panel h-fit">
              ...
              <TransactionForm
                defaultType={transactions[0]?.type ?? "EXPENSE"}
                members={members}
                defaultPaidByMemberId={transactions[0]?.paidByMemberId}
              />
            </section>
            ...
            <section className="panel">
              ...
              {transactions.length === 0 ? (
                <div className="empty-state">
                  ...
                  <p className="font-bold text-slate-700">Henüz kayıt yok</p>
                  <p className="mt-1 text-sm text-slate-400">
                    İlk hareketini sol taraftan ekleyebilirsin.
                  </p>
                </div>
              ) : (
                <ul className="divide-y divide-slate-100">
                  {transactions.map((transaction) => {
                    ...
                    return (
                      <li
                        key={transaction.id}
                        className="transaction-row flex items-center gap-3 py-4 first:pt-0 last:pb-0"
                      >
                        <div
                          className={
                            isIncome
                        ...
                        <div className="min-w-0 flex-1">
                          <p
                            className="transaction-description break-words text-sm font-bold text-slate-800"
                          ...
                          <div className="transaction-meta mt-1 flex flex-wrap items-center gap-x-2 gap-y-1 text-xs text-slate-400">
                            ...
                            <span aria-hidden="true">·</span>
                            <span>
                              {transaction.paidByMember
                                ? `${transaction.paidByMember.name} ${
                                    isIncome ? "aldı" : "ödedi"
                                  }`
                                : "Bölüşüldü"}
                            </span>
                        ...
                        <div className="transaction-actions flex shrink-0 items-center gap-3">
                          <p
                            className={
                              isIncome
                                ? "text-right text-sm font-black text-emerald-700"
                                : "text-right text-sm font-black text-rose-600"
                            }
                          >
                            {isIncome ? "+" : "−"}
                            {currency.format(transaction.amountCents / 100)}
                          </p>
                          <EditTransactionModal
                            transaction={{
                              id: transaction.id,
          ...
          <section className="order-2 mb-8 grid gap-4 sm:grid-cols-3 lg:order-1">
